...
public class EditingScoreListWidget extends ElementListWidget<EditingScoreListWidget.Entry> {
    ...
    public EditingScoreListWidget(MinecraftClient minecraftClient, ScoreEditingScreen parent) {
        super(minecraftClient, parent.width + 25, parent.height, 32, parent.height - 70, 25);
        this.parent = parent;
    ...
    public class Entry extends ElementListWidget.Entry<Entry> {
        ...
        public Entry(ScoreEditingScreen.SingleScore entry, boolean isNewEntry) {
            ...
            this.scoreField = new TextFieldWidget(client.textRenderer, 0, 0, 80, 20, Text.empty()) {
                @Override
            ...
            this.nameField = new TextFieldWidget(client.textRenderer, 0, 0, 100, 20, Text.empty());
        ...
        public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            if (!this.nameField.visible) {
                ...
                EditingScoreListWidget.this.client.textRenderer.draw(matrices, text, x + 20 + 5, y + (float) entryHeight / 2 - (float) EditingScoreListWidget.this.client.textRenderer.fontHeight / 2, 0xFFFFFF);
            }
            this.nameField.setPosition(x + 20 + 5, y);
            this.nameField.render(matrices, mouseX, mouseY, tickDelta);
            this.editButton.setPosition(x, y);
            this.editButton.render(matrices, mouseX, mouseY, tickDelta);
            this.scoreField.setPosition(x + 130, y);
            this.scoreField.render(matrices, mouseX, mouseY, tickDelta);
